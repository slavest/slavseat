...
export const IN_VIEW_DEGREES =
  WHEEL_ITEM_RADIUS * WHEEL_ITEMS_IN_VIEW;
...
const setSlideStyles = (
  ...
  const positionDefault =
    emblaApi.scrollSnapList()[index] * totalRadius;
  ...
  if (loop && isInView(wheelLocation, positionLoopEnd)) {
    inView = true;
    angle =
      -CIRCLE_DEGREES + (slideCount - index) * WHEEL_ITEM_RADIUS;
  }

  if (loop && isInView(wheelLocation, positionLoopStart)) {
    inView = true;
    angle =
      -(totalRadius % CIRCLE_DEGREES) - index * WHEEL_ITEM_RADIUS;
  }
...
export const IosPickerItem: React.FC<PropType> = (props) => {
  ...
  useEffect(() => {
    ...
    emblaApi.on('pointerUp', (emblaApi) => {
      const { scrollTo, target, location } =
