...
export class ObjectStorageService {
  ...
  async save(
    ...
    const putObjectRequest: PutObjectRequest = {
      namespaceName: this.configService.getOrThrow(
    ...
    const putObjectResponse =
      await this.oracleObjectStorageClient.putObject(
        putObjectRequest,
      );
    ...
    if (existObject) {
      const mergedObjcet = this.objectMetaRepository.merge(
        existObject,
        {
          name,
          type,
    ...
    const object = this.objectMetaRepository.create({
      name,
      type,
      isPublic,
    });
  ...
  private async createPreAuthedUrl(
    name: string,
    bucketName: string,
    expiresInSec: number,
  ) {
    ...
    return (
      baseUrl +
      createPreauthedRequest.preauthenticatedRequest.accessUri
    );
  }
}
